  ...
  const signupMutation = useMutation({
    ...
    onError: (error) => {
      if (error instanceof ApiError && error.status === 400) {
        setFormError("Unable to create account with those details. Try another email.");
        return;
      }
  ...
  return (
    <AuthShell
      cta={
        <div className="space-y-3">
          ...
          <p>
            Account creation is live in-app for internal users, and successful signup creates a
            session immediately.
          </p>
        </div>
      }
    ...
    >
      <div className="space-y-2">
        <h2 className="text-2xl font-semibold text-white">Set up your account</h2>
        <p className="text-sm leading-6 text-slate-300">
          This branch stops after authentication. Organization onboarding follows in FE 03.
        </p>
      </div>
      <form className="mt-8 space-y-5" onSubmit={handleSubmit}>
        ...
        <Button className="w-full" isBusy={signupMutation.isPending} type="submit">
          Create account
        </Button>
      </form>
      <div className="mt-6 text-sm text-slate-400">
        <Link className="text-sky-200 transition hover:text-sky-100" to="/login">
